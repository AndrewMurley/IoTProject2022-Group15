...
async function main( )
{
    ...
    // start repetitively calling checkLeak 0.5 seconds
    setInterval(checkLeak, 500);
        ...
        complete: () => {
            ...
            if ( flowing == 1 ) {
                queryApi.queryRows(moved, {
                    ...
                    complete: () => {
                        lastMoved = Date.parse( tempMoved );
                        // compare time elapsed from last movement of gyro to avg
                        timeElapsed = (Date.now() - lastMoved) / 1000;
                        console.log(timeElapsed);
                        console.log("avg=" + avg);
                        // if time elapsed is greater than 1.5 * avg and less than 2 * avg, change leds to yellow
                        if ( timeElapsed >= ( avg * 1.5 ) && timeElapsed < ( avg * 2) ){
                            leak = 2;
                        }
                        // if time elapsed is greater than 2 * avg, change leds to red
                        else if ( timeElapsed >= ( avg * 2 ) ){
                            leak = 1;
                        }
